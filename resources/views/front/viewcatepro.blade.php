                            ...
                            <div class="single-products">
                                ...
                                <div class="product-overlay">
                                    <div class="overlay-content">
                                        @if($item->discount!=0)
                                            <h2 style="font-size: 15px;"><span style="text-decoration: line-through;">{{ number_format($item->price) }} </span><sup>đ</sup></h2>
                                            <h2>{{ number_format($item->price-($item->price*$item->discount/100)) }} <sup style="text-decoration: underline;">đ</sup></h2>
                                        @else
                                            <h2 style="font-size: 20px;">{{ number_format($item->price-($item->price*$item->discount/100)) }} <sup style="text-decoration: underline;">đ</sup></h2>
                                        @endif
                                        <p>{{ $item->name }}</p>
                                        <a href="{{ url('product/'.$item->id) }}" class="btn btn-default add-to-cart"><i class="fa fa-shopping-cart"></i>Thêm vào giỏ hàng</a>
                                    </div>
